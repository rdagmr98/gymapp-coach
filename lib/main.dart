...
import 'package:app_links/app_links.dart'; // Per gestire l'apertura del file
import 'package:flutter/foundation.dart' show kIsWeb;
// Importa questo per gestire il web senza errori
import 'package:url_launcher/url_launcher.dart';

import 'dart:io'; // Per GZip
...
void _inviaLinkUniversale(dynamic client, BuildContext context) async {
  try {
    // 1. Converti in JSON
    String jsonScheda = jsonEncode(client.routine);
    await Clipboard.setData(ClipboardData(text: jsonScheda));

    // 2. Comprimi con GZip
    List<int> gZipBytes = gzip.encode(utf8.encode(jsonScheda));

    // 3. USA base64UrlEncode (fondamentale per gli URL!)
    String base64Data = base64UrlEncode(gZipBytes);
    // Importante: Rimuovi i caratteri '=' alla fine che spesso rompono i link su WhatsApp
    base64Data = base64Data.replaceAll('=', '');

    String url = "https://rdagmr98.github.io/gymschedeimport/?data=$base64Data";
    String messaggio = "🏋️ Ecco la tua scheda: $url";

    if (kIsWeb) {
      await Clipboard.setData(ClipboardData(text: jsonScheda));
      // Su Web apriamo WhatsApp o mostriamo un avviso
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Link COPIATO! Incollalo su WhatsApp.")),
      );
    } else {
      // Su Mobile usiamo lo Share classico
      //await Share.share(messaggio);
    }
  } catch (e) {
...
class _PTDashboardState extends State<PTDashboard> with WidgetsBindingObserver {
    ...
    try {
      ...
      if (data != null && data.text != null) {
        ...
        if (text.startsWith("TIPO:PROGRESSI_GYM")) {
          ...
          if (mounted) _mostraDialogoAssociazione(decoded);
        }
      }
    } catch (e) {
  ...
  void _gestisciFileInEntrata(Uri uri) async {
    try {
      ...
      if (content.startsWith("TIPO:PROGRESSI_GYM")) {
        String jsonStr = content.substring(content.indexOf('\n') + 1);
        _mostraDialogoAssociazione(jsonDecode(jsonStr));
      }
    } catch (e) {
  ...
  void _mostraDialogoAssociazione(dynamic datiLog) {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (c) => AlertDialog(
        title: const Text("Dati Progressi Rilevati"),
        ...
        actions: [
          ...clients.map(
            (atleta) => TextButton(
              onPressed: () {
                setState(() {
                  // Aggiorniamo i log dell'atleta selezionato
                  atleta.performanceLogs = List<Map<String, dynamic>>.from(
                    datiLog is List ? datiLog : [datiLog],
                  );
                });
                _saveData();
                Navigator.pop(c);
...
class _ClientDetailViewState extends State<ClientDetailView> {
  ...
  void _handleIncomingProgressFile(Uri uri) async {
    try {
      ...
      // 1. LETTURA SICURA (Gestisce content:// e file://)
      if (uri.scheme == 'content') {
        // Usiamo readAsBytes dall'URI convertito in file o tramite i canali di sistema
      ...
      // 2. VERIFICA TIPO
      if (content.startsWith("TIPO:PROGRESSI_GYM")) {
        // Cerchiamo l'inizio della lista JSON '['
        int jsonStartIndex = content.indexOf('[');
        if (jsonStartIndex == -1) throw "Formato progressi non valido";

        String jsonProgressi = content.substring(jsonStartIndex);
        List<dynamic> logsRicevuti = jsonDecode(jsonProgressi);

        setState(() {
          // Aggiorniamo i log delle performance del cliente selezionato
          widget.client.performanceLogs = List<Map<String, dynamic>>.from(
            logsRicevuti,
          );
        });

        widget.onUpdate(); // Salva i dati nel DB del PT
        HapticFeedback.mediumImpact(); // Feedback tattile

        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text("✅ Progressi sincronizzati con successo!"),
              backgroundColor: Colors.green,
            ),
          );
        }
      } else {
        debugPrint("Il file ricevuto non è di tipo PROGRESSI_GYM");
      }
    } catch (e) {
      debugPrint("Errore ricezione progressi: $e");
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Errore nell'importazione dei progressi: $e")),
        );
      }
    }
  }
    ...
    children: [
      ...
      Center(
        child: IconButton(
          ...
          onPressed: () => _inviaLinkUniversale(
            widget.client,
            context,
          ), // Passa qui l'oggetto cliente
        ),
      ),
    ],
  );
  ...
  void _condividiSchedaFile() async {
    try {
      // 1. Genera il JSON della routine
      String jsonScheda = jsonEncode(
        widget.client.routine.map((e) => e.toJson()).toList(),
      );

      // 2. CREIAMO IL CONTENUTO DEL FILE TXT
      // Aggiungiamo un'intestazione per far capire all'App Cliente che è una scheda
      String contenutoFile = "TIPO:SCHEDA_GYM\n$jsonScheda";

      if (!kIsWeb && (Platform.isAndroid || Platform.isIOS)) {
        try {
          final tempDir = await getTemporaryDirectory();

          // ... dentro _condividiSchedaFile ...
          final fileName =
              '${widget.client.name.replaceAll(' ', '_')}_scheda.gym'; // Estensione .gym
          ...
          if (mounted) {
            await Share.shareXFiles([
              XFile(
                ...
                name: fileName,
                mimeType:
                    'application/octet-stream', // Più sicuro per estensioni custom
              ),
            ], subject: 'Scheda di ${widget.client.name}');
          }
        } catch (e) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text(
                  "Errore durante la condivisione del file. Prova a copiare il codice manualmente.",
                ),
              ),
            );
          }
        ...
        // Se siamo su Web o piattaforma non supportata, facciamo solo il backup negli appunti
        debugPrint("Condivisione file non supportata su questa piattaforma.");
      }

      // 3. Backup negli appunti (manteniamo solo il JSON puro qui per comodità)
      await Clipboard.setData(ClipboardData(text: jsonScheda));

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Scheda pronta e codice copiato!"),
            backgroundColor: Colors.green,
          ),
        );
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text("Errore generale: $e")));
      }
    }
  }
}
