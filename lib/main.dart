...
class _PTDashboardState extends State<PTDashboard> {
  ...
  void initState() {
    super.initState();
    _loadData();
    _initDeepLinks(); // Inizializza l'ascolto dei file .log
  }

  // --- LOGICA RICEZIONE FILE ---
  void _initDeepLinks() async {
    _appLinks = AppLinks();

    // 1. Gestisce l'apertura se l'app è già in background
    _linkSubscription = _appLinks.uriLinkStream.listen((uri) {
      _gestisciFileInEntrata(uri);
    });

    // 2. Gestisce l'apertura se l'app era chiusa
    final initialUri = await _appLinks.getInitialLink();
    if (initialUri != null) {
      _gestisciFileInEntrata(initialUri);
    }
  }

  void _gestisciFileInEntrata(Uri uri) async {
    try {
      // Converte l'URI del sistema in un file leggibile
      final file = File(uri.toFilePath());
      final content = await file.readAsString();
      final decodedData = jsonDecode(content);

      if (decodedData != null) {
        _mostraDialogoAssociazione(decodedData);
      }
    } catch (e) {
      ScaffoldMessenger.of(
        context,
  ...
  void _mostraDialogoAssociazione(dynamic datiLog) {
    showDialog(
      ...
      builder: (c) => AlertDialog(
        title: const Text("Log Allenamento Ricevuto"),
        content: const Text("A quale atleta vuoi assegnare questi progressi?"),
        actions: [
          ...clients.map(
            (atleta) => TextButton(
              onPressed: () {
                setState(() {
                  atleta.performanceLogs
                      .clear(); // Reset come nel tuo codice originale
                  if (datiLog is List) {
                    atleta.performanceLogs.addAll(
                      List<Map<String, dynamic>>.from(datiLog),
                    );
                  } else {
                    atleta.performanceLogs.add(
                      Map<String, dynamic>.from(datiLog),
                    );
                  }
                });
                _saveData();
                Navigator.pop(c);
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text("Dati aggiornati per ${atleta.name}")),
                );
              },
  ...
  @override
  void dispose() {
    _linkSubscription?.cancel(); // Pulizia per evitare memory leak
    super.dispose();
  }

  // --- METODI ORIGINALI ESISTENTI ---
...
class _ClientDetailViewState extends State<ClientDetailView> {
  ...
  void _condividiSchedaFile() async {
    try {
      ...
      // 2. Logica File (Solo per Android/iOS Fisici)
      if (!kIsWeb && (Platform.isAndroid || Platform.isIOS)) {
        try {
          final tempDir = await getTemporaryDirectory();
          final filePath =
              '${tempDir.path}/${widget.client.name.replaceAll(' ', '_')}_scheda.gym';
          final file = File(filePath);
          await file.writeAsString(jsonScheda, flush: true);
          ...
          if (mounted) {
            await Share.shareXFiles(
              ...
              text:
                  'Ciao! Ecco la tua nuova scheda! Se il file non si apre, incolla il codice che ho copiato negli appunti.',
            );
          }
        } catch (e) {
          debugPrint("Errore generazione file: $e");
        }
      }

      // 3. SEMPRE: Copia negli appunti (per iPhone/Web o backup)
      await Clipboard.setData(ClipboardData(text: jsonScheda));

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Codice scheda copiato negli appunti!"),
